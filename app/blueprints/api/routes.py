...
# ══════════════════════════════════════════════════════════════
# ADMIN — Full Reset (TEMPORARY — remove after first use)
# ══════════════════════════════════════════════════════════════

@api_bp.route('/admin/full-reset', methods=['POST'])
@jwt_required
def api_full_reset():
    """Full application reset via API. Admin only. TEMPORARY endpoint."""
    if not request.api_user.is_manager_or_above():
    ...
    try:
        ...
        deleted['reminders'] = TourStopReminder.query.delete()
        deleted['guestlist'] = GuestlistEntry.query.delete()
        ...
        deleted['crew_slots'] = CrewScheduleSlot.query.delete()
        deleted['external_contacts'] = ExternalContact.query.delete()
        deleted['lineup_slots'] = LineupSlot.query.delete()
        deleted['logistics_assignments'] = LogisticsAssignment.query.delete()
        deleted['promotor_expenses'] = PromotorExpenses.query.delete()
        deleted['local_contacts'] = LocalContact.query.delete()
        deleted['logistics'] = LogisticsInfo.query.delete()
        deleted['document_shares'] = DocumentShare.query.delete()
        ...
        deleted['bands'] = Band.query.delete()
        deleted['user_payment_configs'] = UserPaymentConfig.query.filter(
            UserPaymentConfig.user_id != admin_id).delete(synchronize_session=False)
        deleted['user_professions'] = UserProfession.query.filter(
            UserProfession.user_id != admin_id).delete(synchronize_session=False)
        deleted['travel_cards'] = TravelCard.query.filter(
            TravelCard.user_id != admin_id).delete(synchronize_session=False)
        ...
        # Clean all remaining FK references to users before deletion
        db.session.execute(db.text("DELETE FROM audit_logs WHERE user_id != :aid"), {'aid': admin_id})
        db.session.execute(db.text(
            "UPDATE system_settings SET updated_by_id = NULL WHERE updated_by_id IS NOT NULL AND updated_by_id != :aid"
        ), {'aid': admin_id})
        db.session.execute(db.text(
            "UPDATE organizations SET created_by_id = NULL WHERE created_by_id IS NOT NULL AND created_by_id != :aid"
        ), {'aid': admin_id})
        # Catch any remaining FK refs with raw TRUNCATE on the user table
        deleted['users'] = User.query.filter(
            User.id != admin_id).delete(synchronize_session=False)

        db.session.commit()
        return api_success({'reset': True, 'deleted': {k: v for k, v in deleted.items() if v}})
    except Exception as e:
        db.session.rollback()
        return api_error('internal_error', f'Reset failed: {str(e)}', 500)
